...
public class QrySopScore extends QrySop {
  ...
  public double getScoreDefault(RetrievalModel r, int docid) throws IOException{
  	...
  	Idx IdxObj = new Idx();
  	double tot_len = (double)IdxObj.getSumOfFieldLengths(q_inv.field);
  	double doc_length = (double)IdxObj.getFieldLength(q_inv.field,docid);
  	double prior=ctf/tot_len;
  	
  	return (1500*prior)/(doc_length + 2500) + 0.4*prior;
  }
  /**
  ...
  public double getScoreBM25(RetrievalModel r) throws IOException {
	    ...
	    if (! this.docIteratorHasMatchCache()) {
	      return 0.0;
	    	...
	    	QryIop q_inv = (QryIop) this.args.get(0);
	    	double tf = (double)q_inv.tf,df = (double)q_inv.getDf(),score=0.0;
	    	Idx IdxObj = new Idx();
	    	double k_1 = ((RetrievalModelBM25) r).get_k1();
	    	double b = ((RetrievalModelBM25) r).get_b();
	    	double k_3 = ((RetrievalModelBM25) r).get_k3();
	    	double num_docs = (double)IdxObj.getDocCount(q_inv.field);
	    	double tot_len = (double)IdxObj.getSumOfFieldLengths(q_inv.field);
	    	double average_len = tot_len/num_docs;
	    	double doc_length = (double)IdxObj.getFieldLength(q_inv.field,q_inv.docIteratorGetMatch());
  ...
  public double getScoreIndri(RetrievalModel r) throws IOException{
	  ...
	  if (! this.docIteratorHasMatchCache()) {
	      return 0.0;
	    	...
	    	QryIop q_inv = (QryIop) this.args.get(0);
	    	double tf = (double)q_inv.tf,df = (double)q_inv.getDf(),ctf = (double)q_inv.getCtf();
	    	Idx IdxObj = new Idx();

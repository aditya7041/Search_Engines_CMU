...
public abstract class Qry {
   ...
   */
   ...
   public static final int INVALID_NEARDIS = Integer.MIN_VALUE;
  ...
  private int docIteratorMaxFreq = Qry.INVALID_DOCID;
  private int docInteratorMinFreq = Qry.INVALID_DOCID;
  
  private int nearDistance = Qry.INVALID_NEARDIS;
  ...
  public void appendArg(Qry q) throws IllegalArgumentException {
	...
	/* Adi: One cannot append to the inverted list, so return from here */
    ...
    /*
     *  It the parent is the score operator, then it should be the score of only one inverted
     *  list. The List to append must be QueryIopTerm i.e inverted list of the term retrieved 
     *  from the Lucene index. It cannot be of type QryIop
     */
  ...
  public int docInteratorGetMaxFreq(){
	    if (this.docIteratorHasMaxFreq ()) {
	        return this.docIteratorMaxFreq;
	      } else {
	        throw new IllegalStateException("No matching docid was cached.");
	}
  }
  ...
  /**
   *  An instantiation of docIteratorHasMatchMax that is true if the
   *  query has a document that matches at all the one query argument;
   *  the match is the smallest docid to match; some subclasses like 
   *  SopAnd may choose to use this implementation.
   *  @param r The retrieval model that determines what is a match
   *  @return True if the query matches, otherwise false.
   */
  protected boolean docIteratorHasMatchMax (RetrievalModel r) {
  ...
  protected boolean docIteratorHasMaxFreq() {
	    return (this.docIteratorMaxFreq != Qry.INVALID_DOCID);
	  }
  
  protected int docIteratorGetMaxFreq() {
	    return this.docIteratorMaxFreq;
	  }
  /**

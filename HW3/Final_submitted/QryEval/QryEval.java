   ...
   // Stack<Double> weightStack = new Stack<Double>();
	...
	// Not done yet.  Add the current operator as an argument to
        ...
        if(currentOp instanceof QrySopWand){
        	System.out.println("Adding weights to the Wand Sop Operator of size = " 
        			+ weights.size());
        	((QrySopWand) currentOp).weights = weights;
        }else if(currentOp instanceof QrySopWsum){
        	System.out.println("Adding weights to the Wsum Sop Operator of size = " 
        			+ weights.size());
        	((QrySopWsum) currentOp).weights = weights;
        }
        Qry arg = currentOp;
        	...
        	if(token.equals("/")){
        		...
        		if(tokens.hasMoreTokens()){
        				...
        				if(prev_token.equalsIgnoreCase("#near")){
        					...
        					System.out.println("Window dis = " + dis);
        				}
        		}else{
        			throw new IllegalArgumentException
